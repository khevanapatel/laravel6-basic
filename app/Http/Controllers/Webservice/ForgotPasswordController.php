...
class ForgotPasswordController extends Controller
{
    public function forgot(Request $request)
    {
        ...
        if($user != null){
            ...
            if(count(Mail::failures()) > 0){
                return response()->json(['message' => 'Failed to send password reset email, please try again.', 'data'=> '']); 
            }
            return response()->json(['message' => 'One Time Password (OTP) has been sent to your registered email id, Please Enter OTP here to reset your password.', 'data'=> '']);  
        }
        else{
            ...
            {   
                ...
                {   
                    ...
                    {   
                        ...
                        return response()->json(['message' => 'Invalid OTP. Enter Correct OTP Number.']); 
                    }
                }else
                {
    ...
    public function new_password(Request $request)
    {
          ...
          if($request->password === $request->password_confirmation){
            ...
            return response()->json(['message' => 'Your Password confirmation does not match.']); 
          }
          
        }else{
            return response()->json(['message' => 'This email not valid, Please enter register email']); 
        }
    }
}